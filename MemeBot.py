...
#bot.login(proxy=random.choice(proxies))
bot.login()

bot.acceptIncomingPostDms(['vocabexpansion'],caption_request=False)
...
def postMeme():
    ...
    bot.uploadPhoto(os.getcwd() + '/' + str(post[1]) + '/' + img_files[0], 'Follow @derbgang', request_approval=True,
                    media_id=post[0].mediaid, user=post[0].owner_username)
